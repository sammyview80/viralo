  ...
  return (
    <div className="space-y-4 p-4">
      <div className="flex flex-wrap gap-2">
        <button onClick={onBuildDefault} className="min-h-[44px] rounded-md border px-3 py-1.5 text-sm outline-none transition-colors hover:bg-muted focus-visible:ring-2 focus-visible:ring-[#ff3d6a]">
          Build from ready shots
        </button>
        <button onClick={() => onSave(clips)} disabled={!clips.length}
                className="min-h-[44px] rounded-md border px-3 py-1.5 text-sm outline-none transition-colors hover:bg-muted disabled:opacity-50 focus-visible:ring-2 focus-visible:ring-[#ff3d6a]">
          Save timeline
        </button>
        <button onClick={onVoiceover} className="min-h-[44px] rounded-md border px-3 py-1.5 text-sm outline-none transition-colors hover:bg-muted focus-visible:ring-2 focus-visible:ring-[#ff3d6a]">
          Generate voiceover
        </button>
        <input value={musicPrompt} onChange={(e) => setMusicPrompt(e.target.value)}
               placeholder="Music prompt…"
               className="min-h-[44px] w-full rounded-md border bg-background px-3 py-1.5 text-sm outline-none focus-visible:ring-2 focus-visible:ring-[#ff3d6a] sm:w-44" />
        ...
        <button onClick={onRender} disabled={render.status === "rendering" || !clips.length}
                className="min-h-[44px] rounded-md bg-[#ff3d6a] px-3 py-1.5 text-sm text-white outline-none transition-opacity disabled:opacity-50 focus-visible:ring-2 focus-visible:ring-[#ff3d6a] focus-visible:ring-offset-2">
      ...
      <div className="space-y-2">
        ...
        {clips.map((c, i) => (
          <div key={c.id} className="flex flex-wrap items-center gap-2 rounded-md border p-3 text-sm">
            ...
            <input type="number" min={0} step={0.5} value={c.in_s}
                   ...
                   className="min-h-[44px] w-16 rounded border bg-background px-1 py-0.5 text-xs outline-none focus-visible:ring-2 focus-visible:ring-[#ff3d6a]" />
            ...
            <div className="ml-auto flex gap-1">
              <button onClick={() => move(i, -1)} disabled={i === 0}
                      className="min-h-[44px] min-w-[44px] rounded border px-2 text-xs outline-none transition-colors hover:bg-muted disabled:opacity-30 focus-visible:ring-2 focus-visible:ring-[#ff3d6a]">↑</button>
