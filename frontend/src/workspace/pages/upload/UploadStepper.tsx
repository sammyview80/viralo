        ...
        return (
          <div key={n} className="flex flex-col">
            <button
              ...
              className={cn("flex items-center gap-3 rounded-[11px] px-3 py-2.5 text-left transition", active ? "bg-surface-2" : done ? "hover:bg-surface-2 cursor-pointer" : "cursor-default")}
            >
              <div className={cn(
              ...
              <div className="min-w-0">
                <div className={cn("text-[13px] font-semibold leading-tight", active ? "text-c-text" : done ? "text-c-text-secondary" : "text-c-text-muted")}>{s.label}</div>
                <div className={cn("mt-0.5 text-[11px] leading-tight", active ? "text-c-text-secondary" : "text-c-text-muted")}>{s.sub}</div>
              </div>
            </button>
            {i < UPLOAD_STEPS.length - 1 && (
              <div className={cn("ml-7 h-5 w-[2px] rounded-full", done ? "bg-[#ff3d6a]/30" : "bg-c-border")} />
            )}
          </div>
        );
      })}
