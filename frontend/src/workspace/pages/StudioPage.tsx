  ...
  return (
    ...
    >
      <div className={cn(
        ...
        tpl.bg,
        selected ? "border-[#ff3d6a] shadow-[0_0_0_3px_rgba(255,61,106,.25)]" : "border-white/[.08] group-hover:border-white/25"
      )}>
        {/* top mock title */}
      ...
      <p className={cn("mt-2 text-[12px] font-bold", selected ? "text-[#ff7a9a]" : "text-zinc-400")}>{tpl.label}</p>
    </button>
  );
}
...
function FeatureCheck({ label, checked, onChange, badge }: { label: string; checked: boolean; onChange: () => void; badge?: boolean }) {
  return (
    <button type="button" onClick={onChange} className="flex cursor-pointer items-center gap-2 text-left">
      <span className={cn("grid h-[18px] w-[18px] shrink-0 place-items-center rounded-[5px] border transition", checked ? "border-[#ff3d6a] bg-[#ff3d6a]" : "border-white/20 bg-white/[.04]")}>
      ...
      <span className="grid h-3.5 w-3.5 place-items-center rounded-full border border-white/15 text-[8px] font-bold text-zinc-500">i</span>
    </button>
  );
}
...
function YoutubeImportModal({ onClose, initialUrl = "", prefetched = null }: YoutubeModalProps) {
  ...
  const selectClass = "flex h-[50px] items-center gap-2 rounded-[12px] border border-white/[.08] bg-white/[.025] px-3.5 transition focus-within:border-[#ff3d6a]/50";
  ...
  return createPortal(
    ...
    >
      ...
      <div className="relative flex max-h-[92vh] w-full max-w-[680px] flex-col overflow-hidden rounded-[20px] border border-white/[.09] bg-[#0b1018] shadow-[0_40px_120px_rgba(0,0,0,.7)]">
          ...
          <div className="p-5">
            <div className="mb-3 flex items-center gap-2.5">
              ...
              <div className="min-w-0">
                <h2 className="font-display text-[15px] font-bold text-white">Import from YouTube</h2>
                <p className="text-[11.5px] text-zinc-500">Paste a link to preview the video.</p>
              </div>
              <button onClick={onClose} className="ml-auto grid h-8 w-8 place-items-center rounded-[9px] border border-white/[.08] bg-white/[.04] text-zinc-400 transition hover:bg-white/[.08] hover:text-white">
            ...
            <input
              ...
              className="h-[46px] w-full rounded-[12px] border border-white/[.09] bg-[#060b12] px-3.5 text-[13.5px] font-medium text-zinc-100 outline-none transition placeholder:text-zinc-600 focus:border-[#ff3d6a]/60 focus:ring-4 focus:ring-[#ff3d6a]/15"
            />
        ...
        {ytMetaLoading && (
          <div className="animate-pulse p-5">
            <div className="mb-4 h-[64px] rounded-[14px] border border-white/[.07] bg-white/[.03]" />
            <div className="mb-4 grid grid-cols-2 gap-3">
              <div className="h-[50px] rounded-[12px] border border-white/[.08] bg-white/[.025]" />
              <div className="h-[50px] rounded-[12px] border border-white/[.08] bg-white/[.025]" />
            </div>
            <div className="flex gap-3">
              {Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-[200px] w-[118px] shrink-0 rounded-[14px] bg-white/[.05]" />)}
            </div>
          </div>
        )}
        ...
        {ytMeta && !ytMetaLoading && (
          <>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto p-5">
              ...
              <div className="mb-4 flex items-center gap-3 rounded-[14px] border border-white/[.07] bg-white/[.03] p-3">
                <div className="h-12 w-12 shrink-0 overflow-hidden rounded-[10px] bg-zinc-800">
                  <img
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <p className="truncate text-[13.5px] font-bold text-white">{ytMeta.title || "YouTube video"}</p>
                    <span className="ml-auto shrink-0 rounded-[6px] bg-white/[.06] px-2 py-0.5 text-[10.5px] font-bold text-zinc-400">{ytMeta.quality}</span>
                  </div>
                  <div className="mt-1.5 h-1.5 w-full overflow-hidden rounded-full bg-white/10">
                    <div className="h-full w-full rounded-full bg-emerald-500" />
                  </div>
                  <p className="mt-1.5 text-[11px] font-semibold text-zinc-400">Ready ({fmtClock(ytMeta.duration)})</p>
                </div>
                <span className="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-emerald-500 text-white">
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth={3}><path d="M20 6L9 17l-5-5"/></svg>
                </span>
                <button onClick={onClose} className="grid h-8 w-8 shrink-0 place-items-center rounded-[9px] text-zinc-500 transition hover:bg-white/[.06] hover:text-white">
              ...
              <div className="mb-4 grid gap-3 sm:grid-cols-3">
                <label className={selectClass}>
                  <span className="text-[11px] font-bold uppercase tracking-wide text-zinc-500">Ratio</span>
                  <select value={ratio} onChange={(e) => setRatio(e.target.value)} className="flex-1 cursor-pointer appearance-none bg-transparent text-right text-[13px] font-bold text-zinc-100 outline-none [&>option]:bg-[#0b1018]">
                    {RATIOS.map((r) => <option key={r} value={r}>{r}</option>)}
                  </select>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#71717a" strokeWidth={2.2}><path d="M6 9l6 6 6-6"/></svg>
                </label>
                <label className={selectClass}>
                  <span className="shrink-0 text-[11px] font-bold uppercase tracking-wide text-zinc-500">Length</span>
                  <select value={clipLen} onChange={(e) => setClipLen(e.target.value)} className="flex-1 cursor-pointer appearance-none bg-transparent text-right text-[13px] font-bold text-zinc-100 outline-none [&>option]:bg-[#0b1018]">
                    {CLIP_LENGTHS.map((c) => <option key={c.id} value={c.id}>{c.label}</option>)}
                  </select>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#71717a" strokeWidth={2.2}><path d="M6 9l6 6 6-6"/></svg>
                </label>
                <label className={selectClass}>
                  <span className="shrink-0 text-[11px] font-bold uppercase tracking-wide text-zinc-500">Clips</span>
                  <select value={clipCount} onChange={(e) => setClipCount(e.target.value)} className="flex-1 cursor-pointer appearance-none bg-transparent text-right text-[13px] font-bold text-zinc-100 outline-none [&>option]:bg-[#0b1018]">
              ...
              {/* ── Template tabs ── */}
              <div className="mb-3 flex items-center gap-1 border-b border-white/[.07]">
                {TEMPLATE_TABS.map((t) => (
                  <button
                    key={t}
                    type="button"
              ...
              {tab === "9:16 template" ? (
                <div className="relative">
                  ...
                  <button
                    ...
                    className="absolute right-1 top-[92px] grid h-8 w-8 place-items-center rounded-full border border-white/15 bg-[#0b1018]/90 text-white shadow-lg backdrop-blur transition hover:bg-[#141926]"
                  >
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5}><path d="M9 18l6-6-6-6"/></svg>
                  </button>
                </div>
              ) : (
                <div className="grid place-items-center rounded-[14px] border border-dashed border-white/[.1] bg-white/[.015] py-12 text-center">
                  <p className="text-[12.5px] font-semibold text-zinc-400">{tab === "My template" ? "No saved templates yet" : "Brand templates are a premium feature"}</p>
                </div>
              )}

              {/* ── Feature toggles ── */}
              <div className="mt-4 rounded-[14px] border border-white/[.08] bg-white/[.02] p-4">
                ...
                <div className="mt-4 mb-1.5 flex items-center justify-between">
                  <p className="text-[12px] font-bold text-zinc-300">Find clip moment <span className="font-medium text-zinc-600">(optional)</span></p>
                  <span className="text-[10.5px] font-semibold text-zinc-600">Powered by Spark</span>
                </div>
                <input
                  value={findMoment}
                  onChange={(e) => setFindMoment(e.target.value)}
                  placeholder="Only want specific parts? e.g. when they talk about the chorus."
                  className="h-[44px] w-full rounded-[10px] border border-white/[.08] bg-[#060b12] px-3.5 text-[12.5px] text-zinc-100 outline-none transition placeholder:text-zinc-600 focus:border-[#ff3d6a]/50"
                />
              </div>

              {/* ── Auto schedule ── */}
              <div className="mt-3 flex h-[52px] items-center gap-3 rounded-[14px] border border-white/[.08] bg-white/[.02] px-4">
                ...
                <span className="flex-1 text-[13px] font-semibold text-zinc-200">Auto schedule and post to social account</span>
                <button
                  type="button"
                  onClick={() => setAutoSchedule((v) => !v)}
                  className={cn("relative h-6 w-11 shrink-0 cursor-pointer rounded-full transition-colors", autoSchedule ? "bg-[#ff3d6a]" : "bg-white/[.14]")}
                >
            ...
            <div className="shrink-0 border-t border-white/[.07] bg-[#0b1018] p-4">
              <Button
                disabled={uploading}
...
function RecentTile({ video }: { video: VideoResponse }) {
  ...
  return (
    ...
    >
      <div className={cn("relative aspect-video w-full overflow-hidden rounded-[12px] border border-white/[.08] bg-gradient-to-br shadow-[inset_0_0_0_1px_rgba(255,255,255,.04)] transition group-hover:border-[#ff3d6a]/40", gradFromId(video.id))}>
        {video.thumbnail_url ? (
        ...
        <span className="absolute left-2 top-2 rounded-[6px] bg-black/55 px-2 py-0.5 text-[9.5px] font-bold uppercase tracking-wider text-zinc-200 backdrop-blur-md">
          {sourceLabel(video.source_type)}
        </span>
        {exp.expired ? (
          <span className="absolute left-1/2 top-1/2 flex -translate-x-1/2 -translate-y-1/2 items-center gap-1 rounded-full bg-black/60 px-2.5 py-1 text-[10px] font-bold text-zinc-300 backdrop-blur-md">
      ...
      <p className="mt-2 truncate text-[12.5px] font-bold text-zinc-100 transition group-hover:text-white">{video.title || "Untitled"}</p>
      <p className="mt-0.5 text-[11px] text-zinc-500">{formatFullDate(video.created_at)}</p>
      ...
      >
        ...
        <div className="relative overflow-hidden">
          ...
          <div className="relative mx-auto flex w-full max-w-[860px] flex-col items-center px-4 pb-7 pt-8 text-center">
            ...
            <h1 className="mt-3 font-display text-[34px] font-black leading-[1.08] tracking-[-.02em] text-white sm:text-[42px]">
              Turn your long video into <span className="text-[#ff3d6a]">Viral Clips</span>
            </h1>
            <p className="mt-2.5 text-[13.5px] text-zinc-500">Import · Extract viral moments · Export short-form</p>

            {/* Search / drop bar */}
            <div className={cn(
              "relative mt-7 flex w-full max-w-[620px] items-center rounded-full border bg-[#090e17]/80 pl-5 pr-1.5 py-1.5 transition",
              drag ? "border-[#ff3d6a]/60 shadow-[0_0_0_4px_rgba(255,61,106,.12)]" : "border-white/[.10] shadow-[inset_0_1px_0_rgba(255,255,255,.04)]"
            )}>
              ...
              <input
                ...
                className="min-w-0 flex-1 bg-transparent text-[14px] font-medium text-zinc-100 outline-none placeholder:text-zinc-600"
              />
              <button
                type="button"
            ...
            <div className="mt-4 flex flex-wrap items-center justify-center gap-2.5">
              <button type="button" onClick={pickFile} className="flex cursor-pointer items-center gap-2 rounded-full bg-white px-4 py-2 text-[12.5px] font-bold text-zinc-900 transition hover:bg-zinc-200">
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.2}><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
                Upload local file
              </button>
              <button type="button" className="flex cursor-default items-center gap-2 rounded-full bg-white/90 px-4 py-2 text-[12.5px] font-bold text-zinc-900/80">
              ...
              <button type="button" onClick={() => openYtModal()} className="flex cursor-pointer items-center gap-2 rounded-full bg-white px-4 py-2 text-[12.5px] font-bold text-zinc-900 transition hover:bg-zinc-200">
        ...
        {/* ── Choose source ── */}
        <div className="mx-auto w-full max-w-[1180px] border-t border-white/[.07] px-5 py-6 sm:px-8">
          <div className="mb-5">
            <p className="text-[11px] font-bold uppercase tracking-[.18em] text-zinc-500">Choose source</p>
          </div>

          <div className="grid gap-4 md:grid-cols-3">
            {/* AI Generate — disabled, no-op (does nothing on click) */}
            <div className="flex flex-col rounded-[16px] border border-white/[.06] bg-white/[.015] p-5 opacity-60">
              ...
              <div className="mt-3 flex items-center gap-2">
                <h3 className="font-display text-[15px] font-bold text-white">AI Generate</h3>
                <span className="rounded-full border border-white/[.1] bg-white/[.04] px-2 py-0.5 text-[9.5px] font-bold uppercase tracking-wide text-zinc-500">Soon</span>
              </div>
              <p className="mt-1.5 text-[12px] leading-5 text-zinc-500">Describe a topic or paste a script. AI writes, voices, and assembles clips for you.</p>
            ...
            <button
              ...
              className={cn(
                ...
                drag ? "border-[#ff3d6a]/55 bg-[#ff3d6a]/[.06]" : "border-white/[.08] bg-white/[.02] hover:border-[#ff3d6a]/35 hover:bg-white/[.04]"
              )}
            >
              ...
              <h3 className="mt-3 font-display text-[15px] font-bold text-white">Upload Video</h3>
              <p className="mt-1.5 text-[12px] leading-5 text-zinc-500">Drop any MP4, MOV or WebM — podcast, webinar, tutorial — and clip it into viral shorts.</p>
              <div className="mt-3 flex flex-wrap gap-1.5">
            ...
            <button
              type="button"
              onClick={() => openYtModal()}
              className="flex cursor-pointer flex-col rounded-[16px] border border-white/[.08] bg-white/[.02] p-5 text-left transition hover:border-red-400/35 hover:bg-red-400/[.05]"
            >
              ...
              <h3 className="mt-3 font-display text-[15px] font-bold text-white">YouTube Import</h3>
              <p className="mt-1.5 text-[12px] leading-5 text-zinc-500">Paste any YouTube URL. Preview metadata, choose format, clip with your active recipe.</p>
              <div className="mt-3 flex flex-wrap gap-1.5">
        ...
        <div className="mx-auto w-full max-w-[1180px] border-t border-white/[.07] px-5 py-6 sm:px-8">
          <div className="mb-5 flex items-center justify-between">
            <p className="text-[11px] font-bold uppercase tracking-[.18em] text-zinc-500">Recent projects</p>
          ...
          {recentsLoading ? null : recents.length === 0 ? (
            <div className="rounded-[14px] border border-dashed border-white/[.1] bg-white/[.015] px-5 py-10 text-center">
              <p className="text-[13px] font-semibold text-zinc-400">No projects yet</p>
              <p className="mt-1 text-[12px] text-zinc-600">Upload a video or import from YouTube to get started.</p>
            </div>
          ) : (
