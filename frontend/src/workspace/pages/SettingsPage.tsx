...
export function SettingsPage() {
  return (
    <Shell active="settings">
      ...
      <div className="grid gap-6 lg:grid-cols-2">{["Brand kit", "Team", "Billing", "Notifications", "API keys", "Security"].map((x, i) => <Card key={x} className="p-5"><DotIcon label={x.slice(0, 2).toUpperCase()} tone={i % 2 ? "blue" : "rose"} /><h3 className="mt-4 font-semibold">{x}</h3><p className="mt-2 text-sm text-zinc-500">Manage {x.toLowerCase()} preferences.</p></Card>)}</div>
    </Shell>
  );
}
